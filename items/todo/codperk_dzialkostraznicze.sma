...
public plugin_init()
{
	new const nazwa[]   = "Dzialko Straznicze";
	new const opis[]    = "Mozesz stawiac dzialko [C]";
...
public ham_ItemDeploy_Post(weapon_ent)
{
	...
	if(!is_user_alive(owner) || !ma_perk[owner]) return;
...
public fw_TraceAttack_Building(id, enemy)
{
	...
	if(ma_perk[enemy] && get_user_weapon(enemy) == CSW_KNIFE && pev(id, pev_iuser1) == enemy && pev(id, pev_iuser2) < 100)
	{
		new classname[14]
