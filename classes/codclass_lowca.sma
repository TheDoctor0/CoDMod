...
public shoot_belt(id)
{
	...
	if (!crossbowBelts[id]) {
		cod_show_hud(id, TYPE_DHUD, 218, 40, 67, -1.0, 0.42, 0, 0.0, 2.0, 0.0, 0.0, "Juz wykorzystales wszystkie belty!");

		return;
	}

	if (!get_bit(id, crossbowActive)) {
		cod_show_hud(id, TYPE_DHUD, 218, 40, 67, -1.0, 0.42, 0, 0.0, 2.0, 0.0, 0.0, "Przeladuj kusze na R!");

		return;
	}
	
	if (lastCrossbowBelt[id] + 3.0 > get_gametime()) {
		cod_show_hud(id, TYPE_DHUD, 218, 40, 67, -1.0, 0.42, 0, 0.0, 2.0, 0.0, 0.0, "Belt mozesz wystrzelic raz na 3 sekundy!");
